...
batch_size = 20000
model = CLIPModel(model_name=backbone).cuda()
model.eval()
...
if backbone == "ViT-L/14":
    embeddings = np.zeros((nouns_num, 768))
elif backbone == "RN50":
    embeddings = np.zeros((nouns_num, 1024))
else:
...
# dataset = 'Birdsnap'
# nouns = np.array(class_name[dataset])
# nouns_num = nouns.shape[0]
#
# batch_size = 2048
...
#         nouns_batch = nouns[start:end]
#         with torch.no_grad():
#             prompt = get_prompt(nouns_batch, index)
#             feature = model.encode_text(prompt)
#             features.append(feature.cpu().numpy())
#         if i % 50 == 0:
#             print(f"[Completed {i * batch_size}/{nouns_num}]")
#     features = np.concatenate(features, axis=0)
#     print("Feature shape:", features.shape)
#     np.save("./data/nouns_embedding_prompt_" + dataset + str(index) + ".npy", features)
#
#
# # Multi Prompts
# embeddings = np.zeros((nouns_num, 512))
# for index in range(len(SIMPLE_IMAGENET_TEMPLATES)):
#     embedding = np.load("./data/nouns_embedding_prompt_" + dataset + str(index) + ".npy")
#     embeddings += embedding
# embeddings = embeddings / len(SIMPLE_IMAGENET_TEMPLATES)
# np.save("./data/nouns_embedding_ensemble_" + dataset + ".npy", embeddings)
